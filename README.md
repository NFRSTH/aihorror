# AiHorror ÔÇö Surveillance AI Horror for Fabric 26.2
...
- **Surveillance Horror + Adaptive Stalker** ÔÇö AI remembers where you hide, how you play, and counters it
- **Smiling glitch entity** ÔÇö humanoid with wide unnatural smile / black eyes, 3 textures from your Downloads flickering every 150ms
- **Glitched English** ÔÇö chat inserts `ÔûêÔûôÔûÆÔûæ` and random caps
- **Dark-only hunt (except 5/5)** ÔÇö AI only hunts in dark (`light <=5`) unless intensity `5/5` (no light rule)
- **Whisper + intense music** ÔÇö vanilla sounds pitch-edited (`0.8-1.2├ù`) + custom Ogg Vorbis
- **Glitch blocks** ÔÇö corrupted world is purple/black checker (missing-texture style) + crying obsidian / soul soil
- **Full Story** ÔÇö 7 logs (5+ fragments) in chests with corrupted tape
- **Sleep deprivation** ÔÇö less you sleep, more fear (`ticksSinceSleep` > 48000 adds fear)
- **Death protection** ÔÇö death corruption `off` by default (protect world, toggle via command)
...
- `fearLevel` 0-100 internal, `intensity` 0-5 ÔåÆ `intensityFactor() = /5.0` `AiHorrorConfig.java:60`
...
- Triggers: `ÔûêÔûôÔûÆ` whisper, `AMBIENT_CAVE.value()` 1.5f, time snap, door fake, jumpscare (title + `WARDEN_ROAR` + `BLINDNESS`/`SLOWNESS`/`NAUSEA`), glitch spawn (respects `maxGlitchEntities`), corruption (`maxCorruptionPerTick`), inventory shuffle (gated)
...
- **Renderer** `src/client/java/com/aihorror/client/renderer/GlitchEntityRenderer.java:14` ÔÇö `HumanoidModel` (Zombie) cycling 3 user images:
  - `glitch.png` ÔåÉ `Downloads/glitch-5167543797.png` (278├ù491 ÔåÆ 64├ù64)
  - `glitch_creepy.png` ÔåÉ `glitch_creepy-3778253060.png` (315├ù562 ÔåÆ 64├ù64)
  - `glitch_alt.png` ÔåÉ `glitch_alt-3592543168.png` (314├ù552 ÔåÆ 64├ù64)
...
- Replaces `GRASS/DIRT/STONE/COBBLE/SAND` ÔåÆ `CORRUPTED_BLOCK` (purple/black 16├ù16) / `CRYING_OBSIDIAN` / `SOUL_SOIL`; breaks torches
...
- **Scanner** `aihorror:scanner` ÔÇö `I R I / R C R / I R I` (iron/redstone/compass) scans 64 blocks, shows `CRITICAL/HIGH/MEDIUM` + distance, gives Glowing
- **EMF Reader** `aihorror:emf_reader` ÔÇö `G G G / R I R / I R I` (gold/redstone/iron) scores corruption 1-5 (`NONEÔåÆPARANORMAL`) + `DARKNESS` at 5
- Cooldown `ItemStack` 40 / 20 ticks, `sendSystemMessage`

### 5. Block `src/main/java/com/aihorror/block/ModBlocks.java:1`
- `corrupted_block` ÔÇö 16├ù16 purple/black checker + white cross `textures/block/corrupted_block.png:1` (127B valid PNG), `MapColor.COLOR_BLACK`, `2.0/6.0` `SOUL_SOIL` sound, light 2

### 6. Sounds `src/main/resources/assets/aihorror/sounds.json:1` + `sounds/*.ogg:1`
- `jumpscare.ogg` 17KB, `whisper.ogg` 20KB, `glitch_ambient.ogg` 18KB (3s stream), `static_loop.ogg` 25KB (2s loop) ÔÇö all valid OggS Vorbis `soundfile` 44.1kHz, not 10B header
- Vanilla-edited via pitch `0.8-1.2├ù` in `playSound`

### 7. Mixins + Refmap
- `ServerLevelMixin:7` `src/main/java/com/aihorror/mixin/ServerLevelMixin.java:7` `@Mixin(ServerLevel) tick@HEAD`
- `CameraShakeMixin:10` `src/client/java/com/aihorror/mixin/client/CameraShakeMixin.java:10` `@Mixin(Camera) tick@TAIL` ÔÇö shakes `setPosition(position().add(dx,dy,dz))` when `NAUSEA/DARKNESS` (shader glitch)
...
### 8. Config `src/main/java/com/aihorror/config/AiHorrorConfig.java:1` ÔåÆ `config/aihorror.json`
```json
{
  "enabled": true,
...
- Needs `CORRUPTED_TAPE` in hand + midnight (`13000-19000`) + 4├ù `CRYING_OBSIDIAN` nearby ÔåÆ weakens AI, spawns 20HP glitch to kill
...
- **Icon** `assets/aihorror/icon.png:1` 128├ù128 PNG `89 50 4E 47` (was ZIP `PK`) ÔÇö dark bg purple stripes smiling face
- **Entity** 3├ù 64├ù64 from `Downloads/glitch-*.png` (center-cropped Lanczos)
- **Items** `scanner.png:175B` `emf_reader.png:170B` `corrupted_tape.png:135B` all 16├ù16 (were 76B 1├ù1)
- **Block** `corrupted_block.png:127B` 16├ù16 purple/black (was 103B stub)
